...
func InitCors(r *gin.Engine) *gin.Engine {
	...
	r.Use(cors.New(cors.Config{
		AllowOrigins:     []string{fmt.Sprintf("http://%s:%s", os.Getenv("WEB_PROXY_HOST"), os.Getenv("WEB_PROXY_PORT")),fmt.Sprintf("http://%s:%s", os.Getenv("SERVER_HOST"), os.Getenv("SERVER_PORT"))},
